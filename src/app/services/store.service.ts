...
const INITIAL_STORES: Store[] = [
  ...
  },
  {
    ...
    status: 'active',
    owner: 'Carlos Mendez',
    email: 'carlos@sportsarena.com',
    phone: '+1-555-5678',
    address: '101 Stadium Way',
    city: 'Miami',
    state: 'FL',
    country: 'United States',
    revenue: 33600,
  ...
  },
  {
    ...
    status: 'disabled',
    owner: 'Emma Johnson',
    email: 'emma@beautyvault.com',
    phone: '+1-555-6789',
    address: '202 Glamour Rd',
    city: 'Los Angeles',
    state: 'CA',
    country: 'United States',
    revenue: 0,
...
const STORE_DASHBOARD_DATA: Record<string, StoreDashboardData> = {
  'store-001': {
    ...
    salesByCategory: [
      { name: 'Mobiles & Tablets', amount: 8420.5, percentage: 34, color: '#7C3AED' },
      { name: 'Laptops & Computers', amount: 6120.3, percentage: 25, color: '#EC4899' },
      { name: 'Accessories', amount: 4320.2, percentage: 17, color: '#F59E0B' },
      { name: 'Smart Wearables', amount: 3210.1, percentage: 13, color: '#14B8A6' },
      { name: 'Audio', amount: 2710.4, percentage: 11, color: '#3B82F6' },
    ],
    topProducts: [
    ...
    recentOrders: [
      ...
      { id: '#ORD-7850', customer: 'Maria Garcia', amount: 159.5, status: 'Delivered' },
      { id: '#ORD-7849', customer: 'David Smith', amount: 1249.0, status: 'Processing' },
      { id: '#ORD-7848', customer: 'Emily Davis', amount: 89.99, status: 'Shipped' },
      { id: '#ORD-7847', customer: 'Michael Brown', amount: 549.0, status: 'Processing' },
    ],
    performance: [
      {
        id: 'visitors',
        ...
        trend: 'up',
        tone: 'purple',
        sparkline: [24, 22, 26, 21, 32, 37, 28, 25, 21, 24],
      },
      {
        id: 'views',
        label: 'Page Views',
        ...
        trend: 'up',
        tone: 'pink',
        sparkline: [31, 35, 29, 37, 45, 39, 36, 30, 33],
      },
      {
        id: 'bounce',
        label: 'Bounce Rate',
        ...
        trend: 'down',
        tone: 'orange',
        sparkline: [30, 38, 32, 35, 48, 40, 37, 31, 34],
      },
      {
        id: 'conversion',
        ...
        trend: 'up',
        tone: 'green',
        sparkline: [18, 24, 19, 27, 35, 29, 25, 21, 23],
      },
    ],
  },
};
...
const INITIAL_NOTIFICATIONS: Notification[] = [
  {
    ...
    message: 'Fashion Hub received a $420 order',
    time: '2 min ago',
    unread: true,
    icon: 'order',
  },
  {
    ...
    message: 'TechZone is now active',
    time: '1 hr ago',
    unread: true,
    icon: 'success',
  },
  {
    ...
    message: 'Beauty Vault has 3 items below 10 units',
    time: '3 hr ago',
    unread: false,
    icon: 'warning',
  },
  {
    ...
    message: 'Sports Arena crossed $30K revenue',
    time: 'Yesterday',
    unread: false,
    icon: 'star',
  },
...
export class StoreService {
  ...
  readonly activeStoresCount = computed(
    () => this.storesSignal().filter((s) => s.status === 'active').length,
  );
  readonly disabledStoresCount = computed(
    () => this.storesSignal().filter((s) => s.status === 'disabled').length,
  );
  readonly pendingStoresCount = computed(
    () => this.storesSignal().filter((s) => s.status === 'pending').length,
  );
  readonly totalRevenue = computed(() =>
    this.storesSignal().reduce((acc, s) => acc + s.revenue, 0),
  );
  ...
  addStore(data: Partial<Store>, notify = true): Store {
    ...
    const newStore: Store = {
      ...
      inventoryAllocationLimit: data.inventoryAllocationLimit ?? 1000,
    };
    ...
    if (notify) this.showToast(`Store "${newStore.name}" created successfully.`, 'success');
    return newStore;
  }

  updateStore(id: string, updatedFields: Partial<Store>, notify = true): void {
    this.storesSignal.update((stores) => {
    ...
    if (store && notify) {
      this.showToast(`Store "${store.name}" saved successfully.`, 'success');
    }
  }
  ...
  deleteStore(id: string): void {
    const store = this.storesSignal().find((s) => s.id === id);
    const updated = this.storesSignal().filter((s) => s.id !== id);
    this.saveStores(updated);
    this.storesSignal.set(updated);
    if (this.selectedStoreIdSignal() === id) {
      const nextStoreId = updated[0]?.id ?? '';
      this.selectedStoreIdSignal.set(nextStoreId);
      if (nextStoreId) this.storage.setItem(SELECTED_STORE_KEY, nextStoreId);
      else this.storage.removeItem(SELECTED_STORE_KEY);
    }
    if (store) {
      this.showToast(`Store "${store.name}" deleted.`, 'danger');
    }
  }
  changeSelectedStore(id: string, notify = true): void {
    const store = this.getStoreById(id);
    if (store) {
      this.selectedStoreIdSignal.set(id);
      this.storage.setItem(SELECTED_STORE_KEY, id);
      if (notify) this.showToast(`Switched to ${store.name}`, 'info');
    }
  }
  ...
  private loadStores(): Store[] {
    const storedStores = this.storage.getItem<Store[]>(LS_KEY);
    if (!storedStores) return INITIAL_STORES;

    const needsMigration = storedStores.some(
      (store) => store.inventoryAllocationLimit === undefined,
    );
    const migratedStores = storedStores.map((store) => ({
      ...store,
      inventoryAllocationLimit: store.inventoryAllocationLimit ?? 1000,
    }));
    if (needsMigration) this.storage.setItem(LS_KEY, migratedStores);
  ...
  private loadSelectedStoreId(): string {
    ...
    return persistedId && stores.some((store) => store.id === persistedId)
      ? persistedId
      : (stores[0]?.id ?? '');
  }
  ...
  private buildDashboardFromStore(store: Store): StoreDashboardData {
    ...
    return {
      ...
      revenueOverview: ['May 12', 'May 13', 'May 14', 'May 15', 'May 16', 'May 17', 'May 18'].map(
        (label, index) => ({
          label,
      ...
      topProducts: [
        {
          id: `${store.id}-p1`,
          name: `${store.category.split('&')[0].trim()} Starter Kit`,
          sold: 118,
          revenue: revenue * 0.23,
          imageTone: store.accentColor,
        },
        {
          id: `${store.id}-p2`,
          name: `${store.name} Best Seller`,
          sold: 94,
          revenue: revenue * 0.19,
          imageTone: '#2563EB',
        },
        {
          id: `${store.id}-p3`,
      ...
      recentOrders: [
        {
          ...
          amount: Math.max(aov * 2.4, 89.99),
          status: 'Delivered',
        },
        {
          id: '#ORD-7850',
          customer: 'Maria Garcia',
          amount: Math.max(aov * 1.7, 59.5),
          status: 'Delivered',
        },
        {
          id: '#ORD-7849',
          customer: 'David Smith',
          amount: Math.max(aov * 4.1, 149.0),
          status: 'Processing',
        },
        {
          id: '#ORD-7848',
          customer: 'Emily Davis',
          amount: Math.max(aov * 1.1, 39.99),
          status: 'Shipped',
        },
        {
          id: '#ORD-7847',
          customer: 'Michael Brown',
          amount: Math.max(aov * 2.8, 79.0),
          status: 'Processing',
        },
      ],
      performance: [
        {
          id: 'visitors',
          label: 'Visitors',
          value: store.visitors.toLocaleString('en-US'),
          change: 9.4,
          trend: 'up',
          tone: 'purple',
          sparkline: [18, 23, 19, 26, 31, 27, 34, 30, 32],
        },
        {
          id: 'views',
          label: 'Page Views',
          value: Math.round(store.visitors * 2.18).toLocaleString('en-US'),
          change: 12.1,
          trend: 'up',
          tone: 'pink',
          sparkline: [29, 31, 26, 35, 42, 38, 34, 30, 33],
        },
        {
          id: 'bounce',
          label: 'Bounce Rate',
          ...
          trend: 'down',
          tone: 'orange',
          sparkline: [36, 34, 39, 33, 31, 29, 32, 28, 27],
        },
        {
          id: 'conversion',
          label: 'Conversion Rate',
          ...
          trend: 'up',
          tone: 'green',
          sparkline: [16, 20, 18, 24, 28, 25, 30, 27, 31],
        },
      ],
    };
  }
